import Joi from 'joi'
import { OBJECT_ID_RULE, OBJECT_ID_RULE_MESSAGE, EMAIL_RULE, EMAIL_RULE_MESSAGE } from '~/utils/validator'
...
const CARD_COLLECTION_SCHEMA = Joi.object({
  ...
  comments: Joi.array().items({
    ...
    // Chỗ này lưu ý vì dùng hàm $push để thêm comment nên không
    // set default Date.now() luôn giống hàm insertOne khi create được
...
const createNew = async (data) => {
  try {
    const validateData = await validateCard(data)
 ...
 * Di nhiên cứ lưu comment mới vào cuối mảng cũng được, nhưng nay sẽ học cách để thêm phần tử vào đầu mảng trong mongodb.
